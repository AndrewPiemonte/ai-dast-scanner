...
"""
# ZAP Full Scan Script - Change Log

## Features Added:
- `--list` flag: Saves all available scan rules to a JSON file.
- `--remove` flag: Allows disabling specific scan rules.

## Usage Updates:
- `--list <file>`: Export scan rules to JSON.
- `--remove <rule1,rule2,...>`: Disable specific scan rules.

This update enhances configurability, debugging, and security automation integration.
"""
...
def usage():
    ...
    print('    -list file        Saves a list of all rules to file, does not run any scans')
    print('Options:')
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv, "t:c:u:g:m:n:r:J:w:x:l:hdaijp:sz:P:D:T:IU:", ["hook=", "list=", "remove="])
    ...
    for opt, arg in opts:
        if opt == '-h':
            ...
            list_dst = arg
            list = True
            print(f"DEBUG: --list argument received: {arg}")  # Debugging output
        elif opt == '--remove':
            # Split the provided list and strip whitespace
            new_rules = [rule.strip() for rule in arg.split(',')]  # Convert CSV string to list
            ignore_scan_rules.extend(new_rules)  # Add new rules to existing list
            
            # Add all these rules as "INFO" in the config dictionary
            for rule in ignore_scan_rules:
                config_dict[rule] = 'IGNORE'  # Mark as IGNORE to ignore the rule

            print(f"📌 The following scan rules will be treated as INFO: {ignore_scan_rules}")
    ...
    try:
        ...
        # Handle -list: Get list
        if list:
            active_scans = zap.ascan.scanners()  
            passive_scans = zap.pscan.scanners


            # ✅ Save to a JSON file
            scan_data = {
                "active_scans": active_scans,
                "passive_scans": passive_scans
            }
            print(f"\nWritting to {list_dst}...")
            # Save the report
            with open(list_dst, "w") as f:
                json.dump(scan_data, f, indent=4)

            print(f"\nList saved to {list_dst}")
